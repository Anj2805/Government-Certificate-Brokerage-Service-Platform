...
export default function AdminLogin() {
  ...
  const { login, finalizeLogin, logout } = useAuth();
  ...
  const [loginPhase, setLoginPhase] = useState('credentials'); // credentials, setup, verify
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const response = await login({ ...values, role: selectedRole });
      ...
      // Normal flow (if 2fa isn't forced or returning full user)
      const user = response;
  ...
  const onTwoFactorSubmit = async (e) => {
    ...
    try {
      let finalUser;
      if (loginPhase === 'setup') {
        finalUser = await authApi.setup2FA({ tempToken: twoFactorData.tempToken, totpToken });
      } else {
        finalUser = await authApi.verify2FA({ tempToken: twoFactorData.tempToken, totpToken });
      }
      
      finalizeLogin(finalUser);
      await finishLogin(finalUser.user);
    } catch (error) {
      const msg = error?.response?.data?.message || error?.message || '2FA failed. Please try again.';
      toast.error(msg);
      setErrorMessage(msg);
    }
  };
        ...
        <div className="relative w-full max-w-[480px]">
          ...
          <section className="relative w-full bg-white rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-[#e2e8f0] p-8" aria-labelledby="login-title">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <h1 id="login-title" className="mt-4 text-[26px] font-extrabold text-[#0f294a] tracking-tight">Admin Portal</h1>
              <p className="mt-1 text-[13px] font-medium text-[#6b7280]">Secure access for administrators</p>
            </div>
            ...
            {(loginPhase === 'setup' || loginPhase === 'verify') && (
              <form onSubmit={onTwoFactorSubmit} className="mt-6 space-y-5">
                {loginPhase === 'setup' && (
                  <div className="flex flex-col items-center">
                    <p className="text-[13px] font-medium text-[#6b7280] mb-4 text-center">
                      2FA is required for administrators. Scan this QR code with your authenticator app.
                    </p>
                    <img src={twoFactorData?.qrCode} alt="2FA QR Code" className="w-48 h-48 border rounded-lg mb-2" />
                    <p className="text-[12px] font-mono bg-gray-100 p-2 rounded text-gray-700">
                ...
                <div>
                  <label htmlFor="totp-token" className="block text-left text-[13px] font-bold text-[#374151]">
                    Authentication Code
                  </label>
                  <input
                    id="totp-token"
                    ...
                    className="mt-2 h-12 w-full rounded-lg border border-[#cbd5e1] px-4 text-[14px] font-medium text-[#111827] outline-none focus:border-[#13448a] focus:ring-1 focus:ring-[#13448a]"
                    value={totpToken}
                ...
                <button 
                  type="submit" 
                  className="h-12 w-full rounded-lg bg-[#13448a] hover:bg-[#0c316a] text-[14px] font-bold text-white shadow-md transition-colors"
                >
                  {loginPhase === 'setup' ? 'Verify and Enable 2FA' : 'Verify Code'}
                </button>
                ...
                <button 
                  ...
                  className="mt-3 text-[13px] font-bold text-[#6b7280] hover:text-[#13448a] w-full text-center"
                >
                  Cancel
                </button>
              </form>
            ...
            {/* Registration removed for admins */}
